...
```
crates/
  intwav-core   순수 정수 DSP: 분석, dBFS, 슬라이싱, 게인/페이드/DC, TPDF 디더 (부동소수점 미사용 스캔 완료)
  intwav-codec  WAV (hound) + FLAC (claxon 디코딩 / flac-CLI 인코딩) 정수 입출력 + 메타데이터
  intwav-cli    `intwav` 바이너리: 명령어 분석, 파일 I/O, JSON 보고서, 체크섬
```
