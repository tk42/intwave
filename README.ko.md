...
타임스탬프는 `HH:MM:SS.mmm`, `MM:SS.mmm`, `SS.mmm` 또는 초 단위 숫자로 지정할 수 있습니다.
`trim`은 `--output-format flac|wav`(기본값: 출력 확장자에서 유추, 지정되지 않은 경우 FLAC) 및 JSON 처리 보고서(§13)를 생성하는 `--report <path>`를 허용합니다.
...
```
crates/
  intwav-core   순수 정수 처리: 분석, dBFS, 프레임 슬라이싱 (부동소수점 미사용 스캔 완료)
  intwav-codec  WAV (hound) + FLAC (claxon 디코딩 / flac-CLI 인코딩) 정수 입출력
  intwav-cli    `intwav` 바이너리: 명령어 분석, 파일 I/O, JSON 보고서
```
