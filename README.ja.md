...
crates/
  ...
  intwav-engine  CLI/GUI共有エンジン: 処理実装、フローズンJSONレポート、符号化エラー、検証済みアトミック書き込み、波形ピラミッド (浮動小数点非使用ソース)
  intwav-cli     `intwav` バイナリ: エンジンの上にある薄いフロントエンド
```

`intwav-engine` クレートは、今後のGUI（Tauri + React）の基盤となります。すべての処理は同期かつ呼び出し元駆動（進捗通知 + キャンセル対応）であり、すべての書き込みは検証され（`pcm_verified`）、CLIとGUIが同一のエンジンをそのまま共有します。GUI自体、`intwav-playback` レイヤー、およびシーク可能なストリーミングデコーダは、今後のフェーズとして計画されています。
