...
タイムスタンプは `HH:MM:SS.mmm`、`MM:SS.mmm`、`SS.mmm`、または秒数そのものを指定できます。
`trim` では `--output-format flac|wav`（デフォルトは出力ファイルの拡張子から推論、指定がなければFLAC）および、JSON処理レポート (§13) を出力する `--report <path>` を受け付けます。
...
```
crates/
  intwav-core   整数のみの処理: 解析、dBFS、フレームスライス (浮動小数点の非使用をスキャン済み)
  intwav-codec  WAV (hound) + FLAC (claxon デコード / flac-CLI エンコード) の整数入出力
  intwav-cli    `intwav` バイナリ: コマンド解析、ファイルI/O、JSONレポート
```
