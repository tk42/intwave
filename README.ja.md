...
```
crates/
  intwav-core     整数のみのDSP: 解析、ウィンドウ化無音検出、dBFS、スライシング、ゲイン/フェード/DC、TPDFディザ (浮動小数点非使用スキャン済み)
  intwav-codec    WAV (hound) + FLAC (claxon デコード / flac-CLI エンコード) の整数入出力、メタデータ、ヘッダプローブ
  intwav-engine   CLI/GUI共有エンジン: 処理実装、フローズンJSONレポート、符号化エラー、検証済みアトミック書き込み、一度だけデコードするスクラッチ + 波形ピラミッド、非破壊プロジェクト (.iwproj) + アンドゥ/レンダー (浮動小数点非使用ソース)
  intwav-playback プレビュー再生 (cpal): 整数処理チェーンのプレビュー、デバイス境界でのみ浮動小数点使用 — 保存処理パス外、浮動小数点非使用スキャン対象外
  intwav-cli      `intwav` バイナリ: エンジンの上にある薄いフロントエンド
```

`intwav-engine` クレートは、GUI（Tauri + React）の基盤となります。すべての処理は同期かつ呼び出し元駆動（進捗通知 + キャンセル対応）であり、すべての書き込みは検証され（`pcm_verified`）、CLIとGUIが同一のエンジンをそのまま共有します。`open_source` は大容量ソースを一度だけシーク可能なスクラッチファイルへとデコードし、同時に波形データとPCMハッシュをシングルパスで構築します。`intwav-playback` はそのスクラッチファイルからプレビュー再生を行い、エクスポート時と同じ整数処理チェーンを実行し、最終的なデバイス変換時（ネイティブレート優先、浮動小数点リサンプリングはフォールバック）にのみ浮動小数点を使用します。

## GUI (Tauri + React) — プレビュー

デスクトップ GUI は `app/` に配置されています。エンジンをコマンドとして公開する Tauri v2 バックエンド（`src-tauri/`、重いビルドが CI を遅延させないようコアワークスペースから**分離**されたクレート）と、React + TypeScript フロントエンド（日本語デフォルト、バイリンガル対応）から構成されます。`open_source` （一度だけデコードするスクラッチ + 波形）を通じて WAV/FLAC を開き、波形とフローズンレポートの事実に駆動される **Integer-Safe（整数安全）** ステータスパネルを表示し、trim / gain / export16 / verify をリアルタイムな進捗表示とキャンセル機能付きで実行します — これらすべてを CLI と同じエンジン経由で処理します。

```bash
cd app
npm install
npm run tauri dev     # 開発サーバー起動（デスクトップセッションが必要）
npm run tauri build   # 署名済みアプリのバンドル作成 (macOS/Windows/Linux)
```

フロントエンドは `npm run build` でヘッドレスにビルドでき、バックエンドは `app/src-tauri` 内で `cargo check` によりコンパイル確認が可能です。
