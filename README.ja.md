...
crates/
  ...
  intwav-engine   CLI/GUI共有エンジン: 処理実装、フローズンJSONレポート、符号化エラー、検証済みアトミック書き込み、一度だけデコードするスクラッチ + 波形ピラミッド (浮動小数点非使用ソース)
...
`intwav-engine` クレートは、今後のGUI（Tauri + React）の基盤となります。すべての処理は同期かつ呼び出し元駆動（進捗通知 + キャンセル対応）であり、すべての書き込みは検証され（`pcm_verified`）、CLIとGUIが同一のエンジンをそのまま共有します。`open_source` は大容量ソースを一度だけシーク可能なスクラッチファイルへとデコードし、同時に波形データとPCMハッシュをシングルパスで構築します。`intwav-playback` はそのスクラッチファイルからプレビュー再生を行い、エクスポート時と同じ整数処理チェーンを実行し、最終的なデバイス変換時（ネイティブレート優先、浮動小数点リサンプリングはフォールバック）にのみ浮動小数点を使用します。GUI自体（Tauri + React）の開発が残りのフェーズとなっています。
